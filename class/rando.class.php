...
class rando extends SeedObject
{
	...
	public $table_element = 'rando';

	public $element = 'rando';
...
//		global $user;
	...
	private function getNextNumero()
	{
		...
		$numero = get_next_value($db, $mask, 'rando', 'ref');
...
//		global $user;
	...
	public function getNomUrl($withpicto=0, $get_params='')
	{
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showrando") . '</u>';
	...
	public static function LibStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_DRAFT) { $statustrans='statut0'; $keytrans='randoStatusDraft'; $shortkeytrans='Draft'; }
		if ($status==self::STATUS_VALIDATED) { $statustrans='statut1'; $keytrans='randoStatusValidated'; $shortkeytrans='Validate'; }
		if ($status==self::STATUS_REFUSED) { $statustrans='statut5'; $keytrans='randoStatusRefused'; $shortkeytrans='Refused'; }
		if ($status==self::STATUS_ACCEPTED) { $statustrans='statut6'; $keytrans='randoStatusAccepted'; $shortkeytrans='Accepted'; }
...
class randoDet extends TObjetStd
{
	public $table_element = 'randodet';

	public $element = 'randodet';
