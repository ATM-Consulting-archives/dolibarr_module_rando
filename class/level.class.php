...
class level extends SeedObject
{
	/**
	 * Draft status
	 */
	...
	public function getNomUrl($withpicto=0, $get_params='')
	{
		...
		$result='';
		$level = '<u>' . $langs->trans("Showlevel") . '</u>';
		...
		if ($withpicto) $result.=($link.img_object($level, $picto, 'class="classfortooltip"').$linkend);
	...
	public static function LibStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_REFUSED) { $statustrans='statut5'; $keytrans='levelStatusRefused'; $shortkeytrans='Refused'; }
		if ($status==self::STATUS_ACCEPTED) { $statustrans='statut6'; $keytrans='levelStatusAccepted'; $shortkeytrans='Accepted'; }
...
class levelDet extends TObjetStd
{
	public $table_element = 'leveldet';
	
	public $element = 'leveldet';
